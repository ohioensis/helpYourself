...
**Важно**: в функции должно быть только само действие (максимум — связанные с ним поддействия).
...
Function Declaration — это функция, заданная с `function() {...}`, без всяких переменных и т.п. Предпочтительно задавать именно так, такие функции **можно вызывать даже до их объявления** (вызов может быть выше в коде, чем функция).

### Отладка и тестирование

Очень крутой отладчик — вкладка Source в инструментах разработчика Chrome, [вот тут](http://learn.javascript.ru/debugging-chrome) довольно подробно описано, как им пользоваться.
...
Пробелы удобнее табов (больше вариантов отступов). Вертикальные отступы нужны для облегчения читаемости. **Не больше девяти строк подряд без вертикального отступа**.

> Как правило, лучше располагать функции под кодом, который их использует.

**Комментарии нужны!** Но они не должны отвечать на вопрос, что делает этот код, — хороший код сам на них отвечает. И хорошая функция отвечает на вопрос, что она делает, самим своим названием. Комментарии должны содержать пояснения и объяснять, почему было принято именно такое решение и задача решена именно так, а не иначе.
